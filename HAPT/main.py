...
flags.DEFINE_boolean('fine_tune', False,
    ('Specify whether to fine tune a model. This is only allowed if training is being continued from an existing checkpoint '
     'using --continue_training_from. This will reduce the learning rate by the factor 100.')
)
...
def main(argv):
    ...
    # setup pipeline
    if FLAGS.evaluate_from:
        ds_test, ds_info = datasets.load(only_test_set=True)
    else:
    ...
    if not FLAGS.evaluate_from:
        trainer = Trainer(model, ds_train, ds_val, ds_info['num_classes'], run_paths, fine_tuning=FLAGS.fine_tune, load_checkpoint=load_checkpoint)
        for _ in trainer.train():
            continue
    else:
        loss, accuracy, balanced_accuracy, _ = evaluate(model,
                                                             FLAGS.evaluate_from,
                                                             ds_test,
                                                             ds_info,
                                                             run_paths)
        ...
        template = 'Loss: {}, Accuracy: {}%, Balanced Accuracy: {}%'
